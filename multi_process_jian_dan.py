...
"""
use multi-process to download jian_dan img
...
if __name__ == '__main__':
    logging.info('start to download img %s' % '----------------------------------------')
    start_time = time.time()
    with futures.ProcessPoolExecutor(5) as executor:
        res = (executor.submit(download_one_page, num) for num in [40, 39, 38])
        for future in futures.as_completed(res):
            print(future.result())
    logging.info('it used %f' % (time.time() - start_time))
